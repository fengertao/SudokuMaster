...
import charlie.feng.game.sudokumasterserv.solution.Block;
    ...
    private void checkHiddenCellInRowColumn(Grid grid, Region region, boolean isRow) {
        for (int k = 1; k <= 9; k++) {
            ...
            if (subRegionCount == 1) {
                ...
                grid.blocks[blockId].removeDigit(k, region.subRegion[hiddenSubRegionOffset].cells);
    ...
    private void checkHiddenCellInBlock(Grid grid, Block block) {
        for (int k = 1; k <= 9; k++) {
            ...
            }
            {
                ...
                if (subRegionCount == 1) {
                    grid.rows[block.blockR * 3 + hiddenSubRegionOffset].removeDigit(k, block.subRows[hiddenSubRegionOffset].cells);
                }
            }
            {
                int subRegionCount = 0;
                ...
                if (subRegionCount == 1) {
                    grid.columns[block.blockC * 3 + hiddenSubRegionOffset].removeDigit(k, block.subColumns[hiddenSubRegionOffset].cells);
                    //must return here because change always done, some key cell maybe changed.
                    //                    return;
                }
            }
